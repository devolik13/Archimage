...
async function showArenaFormation() {
    ...
    // Функция для получения названий заклинаний
    function getSpellNames(spellIds) {
        ...
        const spellNames = {
            ...
            'inferno': '🔥 Инферно',
            'meteor': '☄️ Метеор',
            'fire_shield': '🛡️ Огненный щит',
            // Водные
            'ice_shard': '❄️ Ледяной осколок',
            'frost_nova': '❄️ Ледяная волна',
            'water_heal': '💚 Исцеление',
            'ice_barrier': '🛡️ Ледяной барьер',
            // Земляные
            'stone_throw': '🪨 Каменный бросок',
            'earthquake': '🌍 Землетрясение',
            'bark_armor': '🛡️ Кора',
            // Воздушные
            'wind_blade': '🌪️ Ветряной клинок',
            'tornado': '🌪️ Торнадо',
            'wind_shield': '🛡️ Воздушный щит'
        };
        
        return spellIds.map(id => spellNames[id] || id);
    }
    ...
    try {
        ...
        // Локальная функция для добавления в расстановку
        window.addToArenaFormation = function(wizardId) {
            if (window.userData && window.userData.formation) {
                // Находим первую свободную позицию
                const freeIndex = window.userData.formation.indexOf(null);
                if (freeIndex !== -1) {
                    // Удаляем мага из других позиций если он уже есть
                    const currentIndex = window.userData.formation.indexOf(wizardId);
                    if (currentIndex !== -1) {
                        window.userData.formation[currentIndex] = null;
                    }
                    window.userData.formation[freeIndex] = wizardId;
                    showArenaFormation(); // Перерисовываем
                }
            }
        };
        ...
        for (let i = 0; i < 5; i++) {
            ...
            formationHTML += `
                ...
                " onclick="${wizard ? `removeFromArenaFormation(${i})` : ''}">
                    ${wizard ? `
                        <div style="text-align: center;">
                            ...
                            ${wizard.spells && wizard.spells.length > 0 ? 
                                `<div style="font-size: 8px; color: #ffa500; margin-top: 2px;">🔮 ${wizard.spells.length} зак.</div>` : 
        ...
        wizards.forEach(wizard => {
            ...
            availableWizardsHTML += `
                ...
                " onclick="${!isAssigned ? `addToArenaFormation('${wizard.id}')` : ''}">
                    ...
                    ${wizard.spells && wizard.spells.length > 0 ? 
                        `<div style="font-size: 8px; color: #ffa500;">🔮×${wizard.spells.length}</div>` : 
                        ''
                    }
        ...
        container.innerHTML = `
            ...
            <div style="margin-bottom: 20px;">
                <h4 style="color: #aaa; font-size: 14px;">Боевые позиции:</h4>
            ...
            <div>
                <h4 style="color: #aaa; font-size: 14px;">Доступные маги (${wizards.length}):</h4>
