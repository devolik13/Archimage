-- Migration 062: Protect time_currency_base from being decreased by update_player_safe
-- Prevents race conditions where auto-save overwrites a higher server value
-- (e.g., after add_time_currency or auto_claim_trial_reward RPC).
-- Only spend_time_currency RPC can decrease the balance.
...
DECLARE
    ...
    new_points INTEGER;
    current_time_base INTEGER;
    new_time_base INTEGER;
BEGIN
    -- Anti-cheat: airdrop_points can only increase
    SELECT airdrop_points, COALESCE(time_currency_base, time_currency, 0)
    INTO current_points, current_time_base
    ...
    -- Anti-rollback: time_currency_base can only increase via update_player_safe
    -- (decreasing is done through spend_time_currency RPC)
    new_time_base := COALESCE((p_data->>'time_currency_base')::INTEGER, current_time_base);
    IF new_time_base < current_time_base THEN
        RAISE NOTICE 'time_currency_base: attempt to decrease % -> %, keeping %', current_time_base, new_time_base, current_time_base;
        new_time_base := current_time_base;
    END IF;
    ...
    SET
        ...
        buildings = CASE
            ...
            ELSE buildings
        END,
        time_currency_base = new_time_base,
