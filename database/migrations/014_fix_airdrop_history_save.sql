-- Migration: Fix airdrop_history saving in RPC
-- Date: 2025-12-10
-- Description: Fix COALESCE logic for airdrop_history to prevent empty array overwrite
...
BEGIN
    ...
    SET
        ...
        -- ИСПРАВЛЕНИЕ: Всегда обновляем историю если она передана и не NULL
        airdrop_history = CASE
            WHEN p_data ? 'airdrop_history' AND p_data->'airdrop_history' IS NOT NULL
            THEN p_data->'airdrop_history'
            ELSE airdrop_history
        END,
...
-- Комментарий
COMMENT ON FUNCTION update_player_safe(bigint, jsonb) IS 'Безопасное обновление данных игрока. Airdrop points защищены от уменьшения. Airdrop history обновляется всегда если передана.';
