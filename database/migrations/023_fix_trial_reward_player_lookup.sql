-- ============================================
-- FIX: auto_claim_trial_reward uses wrong column for player lookup
-- Bug: WHERE id = p_player_id (id is UUID, p_player_id is telegram_id BIGINT)
-- Fix: WHERE telegram_id = p_player_id
-- ============================================

-- Пересоздаём функцию с исправлением
...
BEGIN
    ...
    -- FIX: Начисляем время игроку (используем telegram_id, а не id)
    UPDATE players
...
-- Также исправляем claim_trial_reward (старая версия с таблицей)
