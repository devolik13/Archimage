-- Миграция 024: RPC функция для начисления реферальных наград
-- Нужна SECURITY DEFINER чтобы обойти RLS при обновлении чужой записи

-- Функция для обработки реферала и начисления наград обоим игрокам
...
BEGIN
    ...
    -- Начисляем награду рефереру (SECURITY DEFINER позволяет обновить чужую запись)
    UPDATE players
    SET
...
GRANT EXECUTE ON FUNCTION process_referral TO anon;
