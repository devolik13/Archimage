-- Migration 019: Add Training Dummy Progress
-- Adds training dummy progress tracking for players

-- Add training_dummy_progress column to players table
ALTER TABLE players ADD COLUMN IF NOT EXISTS training_dummy_progress JSONB DEFAULT NULL;

-- Update RPC function to include training_dummy_progress
...
COMMENT ON COLUMN players.training_dummy_progress IS 'Прогресс тренировочного полигона (испытание): попытки, урон, история';
