...
async function checkDailyLoginReward() {
    ...
    // КРИТИЧНО: Сохраняем daily_login в БД ПЕРВЫМ (до начисления наград)
    // Это предотвращает дубль: если игрок закроет приложение после начисления,
    // но до сохранения daily_login — при перезаходе last_reward_date уже будет в БД
    try {
        if (window.dbManager && window.dbManager.supabase) {
            await window.dbManager.supabase.rpc('update_player_safe', {
                ...
                p_data: { daily_login: dailyData }
            });
            console.log('✅ daily_login сохранён в БД (до начисления наград)');
        }
    } catch (err) {
        // Если не удалось сохранить daily_login — НЕ выдаём награду,
        // чтобы не создать ситуацию "награда есть, а защита от дубля нет"
        console.error('❌ Не удалось сохранить daily_login, награда отменена:', err);
        window._isCheckingDailyLogin = false;
        return;
    }

    // Теперь безопасно начисляем награды (daily_login уже в БД)
    ...
    // Финальное сохранение всех данных (время + BPM + остальное)
    try {
        if (window.dbManager && typeof window.dbManager.savePlayer === 'function') {
            await window.dbManager.savePlayer(window.userData);
            console.log('✅ Все данные ежедневной награды сохранены в БД');
        }
    } catch (err) {
        console.error('❌ Ошибка финального сохранения (награда уже начислена):', err);
    }
